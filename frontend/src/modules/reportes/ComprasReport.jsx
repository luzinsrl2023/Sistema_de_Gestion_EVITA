...
import { Download, FileSpreadsheet, FileText, RefreshCw, ShoppingCart } from 'lucide-react'
...
import { useComprasReport } from '../../hooks'
import { useChartTheme } from '../../hooks/useChartTheme'

export default function ComprasReport() {
  const [timeRange, setTimeRange] = useState(() => localStorage.getItem('report:compras:timeRange') || 'yearly')
  ...
  const { 
    ...
    error, 
    refresh 
  } = useComprasReport(timeRange)
  
  const { colors, seriesColors, getColorByIndex } = useChartTheme()

  // Efecto para guardar preferencias
  React.useEffect(() => { 
    localStorage.setItem('report:compras:timeRange', timeRange) 
  }, [timeRange])

  // Handlers optimizados con useCallback
  const handleExportExcel = useCallback(() => {
    if (!comprasMes) return
    
    const data = comprasMes.map(item => ({
      Mes: item.mes,
  ...
  const handleExportPDF = useCallback(() => {
    ...
    const head = ['Mes', 'Compras', 'Cantidad']
    const body = comprasMes.map(item => [
      item.mes,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Cantidad de Compras</p>
            </div>
          </div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">
                {estadisticas?.promedioCompra || '$0'}
              </p>
              <p className="text-sm text-gray-400">Promedio por Compra</p>
            </div>
          </div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Compras del Mes</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Gráfico de Compras Mensuales con TradingView */}
        <TimeSeriesChart
          ...
          subtitle="Evolución de compras por mes"
        ...
        <CustomPieChart
          ...
          subtitle="Compras por categoría de productos"
          colorScheme="default"
        />
      </div>
    </div>
